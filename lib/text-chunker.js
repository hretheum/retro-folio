...
function createChunksWithOverlap(sentences, maxTokens, overlap) {
    ...
    for (let i = 0; i < sentences.length; i++) {
        ...
        const sentenceTokens = (0, openai_1.estimateTokens)(sentence);
        ...
        if (sentenceTokens > maxTokens) {
            ...
            for (const word of words) {
                const wordTokenCount = (0, openai_1.estimateTokens)(word + ' ');
            ...
            for (let j = currentChunk.length - 1; j >= 0 && overlapTokens < overlap; j--) {
                ...
                const overlapSentenceTokens = (0, openai_1.estimateTokens)(overlapSentence);
...
function chunkText(text, options = {}) {
    ...
    const totalTokens = (0, openai_1.estimateTokens)(text);
    ...
    const sentences = opts.preserveSentences
        ...
        : [text];
    // Create chunks with overlap
    const chunkTexts = createChunksWithOverlap(sentences, opts.maxTokens, opts.overlap);
